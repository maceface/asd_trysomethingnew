...
/*$('ajaxJSON').on("click", function() {
	$.ajax({
		...
		success: function(data) {
			console.log(data);
			$.each(data.rows, function(index, logs){
				console.log(logs);
				var gDate = logs.value.gDate;
				var gWhat = logs.value.gWhat;
				var gColor = logs.value.gColor;
				var checked = logs.value.checked;
				var gTime = logs.vaulue.gTime;
			...
			$("#loggingList").listview("refresh);
			alert("Loading JSON Logs");
		}
